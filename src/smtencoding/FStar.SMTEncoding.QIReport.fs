...
let split_z3_stats (header_str : string) (fields : list<(string * string)>) (stats : list<(string * string)>) (rlimit : int) (c : option<string>) : unit =
    let header_len : int = String.length header_str in
    let (fields_str , fields_len) : string * int = prettyprint_table [PrettyRight ; PrettyLeft] (List.map (fun ((s , t) : string * string) -> [s ; t]) fields) in
    let (stats_str , stats_len) : string * int =
        match c with
            | Some cs ->
                let used : int = int_of_string cs in
                let arith_fields = ["arith-pseudo-nonlinear" ;
                    "arith-pivots" ;
                    "arith-offset-eqs" ;
                    "arith-fixed-eqs" ;
                    "arith-eq-adapter" ;
                    ...
                    "arith-assert-diseq" ]
                in
                let quant_fields = ["quant-instantiations" ;
                    "max-generation"]
                in
                let logic_fields = ["propagations" ;
                    "mk-clause" ;
                    "mk-bool-var" ;
                    "minimized-lits" ;
                    "interface-eqs" ;
                    ...
                    "del-clause" ;
                    "decisions" ;
                    "datatype-accessor-ax" ;
                    "conflicts" ;
                    "binary-propagations" ;
                    "num-checks" ]
                in
                let memory_fields = ["num-allocs"] in
                let ignore_fields = ["memory" ; "max-memory" ; "time" ; "rlimit-count"] in
                let fltr (f : string -> bool) ((otr , ofs) : list<(string * string)> * list<(string * string)>) ((ik ,iv) : string * string) : list<(string * string)> * list<(string * string)> =
                    if f ik then ((ik , iv) :: otr , ofs) else (otr , (ik , iv) :: ofs)
                in
                let (_ ,useful_stats) : list<(string * string)> * list<(string * string)> =
                    List.fold_left (fltr (fun (s : string) -> List.contains s ignore_fields)) ([] , []) stats
                in
                let (arith_stats , rest_stats) : list<(string * string)> * list<(string * string)> =
                    List.fold_left (fltr (fun (s : string) -> List.contains s arith_fields)) ([] , []) useful_stats
                in
                let (quant_stats , rest_stats') : list<(string * string)> * list<(string * string)> =
                    List.fold_left (fltr (fun (s : string) -> List.contains s quant_fields)) ([] , []) rest_stats
                in
                let (logic_stats , rest_stats'') : list<(string * string)> * list<(string * string)> =
                    List.fold_left (fltr (fun (s : string) -> List.contains s logic_fields)) ([] , []) rest_stats'
                in
                let (memory_stats , other_stats) : list<(string * string)> * list<(string * string)> =
                    List.fold_left (fltr (fun (s : string) -> List.contains s memory_fields)) ([] , []) rest_stats''
                in
                let arith_stats' : list<(string * int)> = List.map (fun ((s , t) : string * string) -> (s , (int_of_string t))) arith_stats in
                let quant_stats' : list<(string * int)> = List.map (fun ((s , t) : string * string) -> (s , (int_of_string t))) quant_stats in
                let logic_stats' : list<(string * int)> = List.map (fun ((s , t) : string * string) -> (s , (int_of_string t))) logic_stats in
                let memory_stats' : list<(string * int)> = List.map (fun ((s , t) : string * string) -> (s , (int_of_string t))) memory_stats in
                let other_stats' : list<(string * int)> = List.map (fun ((s , t) : string * string) -> (s , (int_of_string t))) other_stats in
                let sm (l : list<(string * int)>) : int = List.fold_left (fun (x : int) ((s , y) : string * int) -> x + y) 0 l in
                let arith_total : int = sm arith_stats' in
                let quant_total : int = sm quant_stats' in
                let logic_total : int = sm logic_stats' in
                let memory_total : int = sm memory_stats' in
                let other_total : int = sm other_stats' in
                let comp ((s1 , i1) : string * int) ((s2 , i2) : string * int) : int = i2 - i1 in
                let arith_list : list<(int * string * int * int)> =
                    (1 , "arithmetics" , arith_total , used) :: (List.map (fun ((s , n) : string * int) -> (2 , s , n , arith_total)) (sort_with comp arith_stats'))
                in
                let quant_list : list<(int * string * int * int)> =
                    (1 , "quantifiers" , quant_total , used) :: (List.map (fun ((s , n) : string * int) -> (2 , s , n , quant_total)) (sort_with comp quant_stats'))
                in
                let logic_list : list<(int * string * int * int)> =
                    (1 , "quantifiers" , logic_total , used) :: (List.map (fun ((s , n) : string * int) -> (2 , s , n , logic_total)) (sort_with comp logic_stats'))
                in
                let memory_list : list<(int * string * int * int)> =
                    (1 , "memory" , memory_total , used) :: (List.map (fun ((s , n) : string * int) -> (2 , s , n , memory_total)) (sort_with comp memory_stats'))
                in
                let other_list : list<(int * string * int * int)> =
                    (1 , "other" , other_total , used) :: (List.map (fun ((s , n) : string * int) -> (2 , s , n , other_total)) (sort_with comp other_stats'))
                in
                let final_list : list<(int * string * int * int)> =
                    (0 , "used rlimit" , used , rlimit) :: (arith_list @ quant_list @ memory_list @ other_list)
                in
                let to_string_list ((k , s , n , t) : int * string * int * int) : list<string> =
                    let r : float = if t <> 0 then (float_of_int32 n) / (float_of_int32 t) * 100.0 else 0.0 in
                    [(repeat k "    ") ^ s ; (string_of_int n) ^ " / " ^ (string_of_int t) ; "(" ^ (string_of_float r) ^ "%)"]
                in
                prettyprint_table [PrettyLeft ; PrettyMark "/" ; PrettyLeft] (List.map to_string_list final_list)
            | None -> ("Resource split could not be computed" , 0)
    in
    let line : string = (repeat (max header_len (max fields_len stats_len)) "-") ^ "\n" in
    print (line ^ header_str ^ "\n" ^ line ^ fields_str ^ "\n" ^ line ^ stats_str ^ "\n" ^ line ^ "\n") []
