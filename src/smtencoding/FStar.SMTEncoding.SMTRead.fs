module FStar.SMTEncoding.SMTRead

open Microsoft.FSharp.Text.Lexing
open FStar.SMTEncoding.SMTSexpr
module SMTParser = FStar.SMTEncoding.SMTParser
module SMTLexer = FStar.SMTEncoding.SMTLexer

let parseSMT (s : string) : stmt list =
    let lexbuf = LexBuffer<char>.FromString s
    lexbuf.EndPos <- lexbuf.EndPos.NextLine
    try
        SMTParser.start SMTLexer.tokenize lexbuf
        with e ->
