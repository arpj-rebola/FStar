...
let uu____161 = (FStar_ST.op_Bang x)
in (uu____161 + (Prims.parse_int "1")))
in (FStar_ST.op_Colon_Equals x uu____159));
(
...
let uu____260 = (FStar_ST.op_Bang x)
in (Prims.string_of_int uu____260))
in (Prims.strcat reserved_prefix uu____258))
in ((uu____256), (r)))
in (mk_ident uu____250));
)))
...
let uu____412 = (FStar_List.map text_of_id ns)
in (FStar_All.pipe_right uu____412 text_of_path))
...
let uu____432 = (FStar_Util.prefix ids)
in (match (uu____432) with
| (ns, id1) -> begin
(

let uu____445 = uu____432
in (lid_of_ns_and_id ns id1))
end)))


let lid_of_str : Prims.string  ->  lident = (fun str -> (

let uu____460 = (FStar_List.map id_of_text (FStar_Util.split str "."))
in (lid_of_ids uu____460)))
...
let uu___80_539 = l
in {ns = uu___80_539.ns; ident = (

let uu___81_541 = l.ident
in {idText = uu___81_541.idText; idRange = r}); nsstr = uu___80_539.nsstr; str = uu___80_539.str}))
...
let uu____556 = (range_of_lid l)
in (lid_of_path (FStar_List.append path ((s)::[])) uu____556))))
...
let uu____572 = (path_of_ns lid.ns)
in (

let uu____576 = (

let uu____580 = (text_of_id lid.ident)
in (uu____580)::[])
in (FStar_List.append uu____572 uu____576)))
in (FStar_All.pipe_left (FStar_String.concat "_") uu____568)))
...
let uu____606 = (path_of_lid lid)
in (text_of_path uu____606)))
