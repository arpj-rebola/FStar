...
let mk_width : Prims.string  ->  width FStar_Pervasives_Native.option = (fun uu___281_4103 -> (match (uu___281_4103) with
| "UInt8" -> begin
...
let mk_bool_op : Prims.string  ->  op FStar_Pervasives_Native.option = (fun uu___282_4125 -> (match (uu___282_4125) with
| "op_Negation" -> begin
...
let mk_op : Prims.string  ->  op FStar_Pervasives_Native.option = (fun uu___283_4155 -> (match (uu___283_4155) with
| "add" -> begin
...
let uu___289_4356 = env
in {names = ({pretty = x})::env.names; names_t = uu___289_4356.names_t; module_name = uu___289_4356.module_name}))


let extend_t : env  ->  Prims.string  ->  env = (fun env x -> (

let uu___290_4370 = env
in {names = uu___290_4370.names; names_t = (x)::env.names_t; module_name = uu___290_4370.module_name}))
...
let find : env  ->  Prims.string  ->  Prims.int = (fun env x -> (FStar_All.try_with (fun uu___292_4409 -> (match (()) with
| () -> begin
(FStar_List.index (fun name -> (Prims.op_Equality name.pretty x)) env.names)
end)) (fun uu___291_4416 -> (

let uu____4418 = (FStar_Util.format1 "Internal error: name not found %s\n" x)
in (failwith uu____4418)))))


let find_t : env  ->  Prims.string  ->  Prims.int = (fun env x -> (FStar_All.try_with (fun uu___294_4438 -> (match (()) with
| () -> begin
(FStar_List.index (fun name -> (Prims.op_Equality name x)) env.names_t)
end)) (fun uu___293_4447 -> (
...
in (FStar_All.try_with (fun uu___296_4856 -> (match (()) with
| () -> begin
((
...
end)) (fun uu___295_4872 -> (match (uu___295_4872) with
| e -> begin
((
...
and translate_flags : FStar_Extraction_ML_Syntax.meta Prims.list  ->  flag Prims.list = (fun flags1 -> (FStar_List.choose (fun uu___284_4944 -> (match (uu___284_4944) with
...
let uu____4959 = (FStar_List.choose (fun uu___285_4966 -> (match (uu___285_4966) with
...
| {FStar_Extraction_ML_Syntax.mllb_name = name; FStar_Extraction_ML_Syntax.mllb_tysc = FStar_Pervasives_Native.Some (tvars, t0); FStar_Extraction_ML_Syntax.mllb_add_unit = uu____5034; FStar_Extraction_ML_Syntax.mllb_def = e; FStar_Extraction_ML_Syntax.mllb_meta = meta; FStar_Extraction_ML_Syntax.print_typ = uu____5037} when (FStar_Util.for_some (fun uu___286_5042 -> (match (uu___286_5042) with
...
let rec find_return_type = (fun eff i uu___287_5179 -> (match (uu___287_5179) with
...
in (FStar_All.try_with (fun uu___298_5268 -> (match (()) with
| () -> begin
(
...
end)) (fun uu___297_5293 -> (match (uu___297_5293) with
| e -> begin
(
...
and translate_width : (FStar_Const.signedness * FStar_Const.width) FStar_Pervasives_Native.option  ->  width = (fun uu___288_8252 -> (match (uu___288_8252) with
| FStar_Pervasives_Native.None -> begin
CInt
end
