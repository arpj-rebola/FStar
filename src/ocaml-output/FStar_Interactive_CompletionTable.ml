...
let rec heap_merge_pairs : 'Auu____287 . ('Auu____287  ->  'Auu____287  ->  Prims.int)  ->  'Auu____287 heap Prims.list  ->  'Auu____287 heap = (fun cmp uu___86_310 -> (match (uu___86_310) with
| [] -> begin
EmptyHeap
end
| (h)::[] -> begin
...
let heap_peek : 'Auu____355 . 'Auu____355 heap  ->  'Auu____355 FStar_Pervasives_Native.option = (fun uu___87_364 -> (match (uu___87_364) with
| EmptyHeap -> begin
...
let heap_pop : 'Auu____384 . ('Auu____384  ->  'Auu____384  ->  Prims.int)  ->  'Auu____384 heap  ->  ('Auu____384 * 'Auu____384 heap) FStar_Pervasives_Native.option = (fun cmp uu___88_411 -> (match (uu___88_411) with
| EmptyHeap -> begin
FStar_Pervasives_Native.None
end
| Heap (v1, hh) -> begin
...
let push_nodup : 'Auu____500 . ('Auu____500  ->  Prims.string)  ->  'Auu____500  ->  'Auu____500 Prims.list  ->  'Auu____500 Prims.list = (fun key_fn x uu___89_523 -> (match (uu___89_523) with
| [] -> begin
(x)::[]
end
| (h)::t -> begin
...
let rec add_priorities : 'Auu____557 . Prims.int  ->  (Prims.int * 'Auu____557) Prims.list  ->  'Auu____557 Prims.list  ->  (Prims.int * 'Auu____557) Prims.list = (fun n1 acc uu___90_589 -> (match (uu___90_589) with
| [] -> begin
acc
end
| (h)::t -> begin
...
let uu___92_3265 = tr1
in {bindings = uu___92_3265.bindings; namespaces = namespaces}))))
...
let uu___93_3318 = tr1
in (

let uu____3321 = (names_insert tr1.bindings id1 v1)
in {bindings = uu____3321; namespaces = uu___93_3318.namespaces})))))
...
let uu___94_3454 = tr1
in {bindings = (ImportedNames (((label), (inc.bindings))))::tr1.bindings; namespaces = uu___94_3454.namespaces}))))
...
let uu___95_3507 = tr1
in {bindings = uu___95_3507.bindings; namespaces = (ImportedNames (((label), (inc.namespaces))))::tr1.namespaces}))))
...
let rec aux = (fun acc imports name_collections1 -> (FStar_List.fold_left (fun acc1 uu___91_3720 -> (match (uu___91_3720) with
| Names (bt) -> begin
(
...
let uu___99_4685 = tbl
in (

let uu____4686 = (trie_open_namespace tbl.tbl_lids host_query included_query)
in {tbl_lids = uu____4686; tbl_mods = uu___99_4685.tbl_mods}))
end))
