...
let mlconst_of_const : FStar_Range.range  ->  FStar_Const.sconst  ->  FStar_Extraction_ML_Syntax.mlconstant = (fun p c -> (FStar_All.try_with (fun uu___280_132 -> (match (()) with
| () -> begin
(mlconst_of_const' c)
end)) (fun uu___279_135 -> (
...
let udelta_unfold : FStar_Extraction_ML_UEnv.uenv  ->  FStar_Extraction_ML_Syntax.mlty  ->  FStar_Extraction_ML_Syntax.mlty FStar_Pervasives_Native.option = (fun g uu___272_444 -> (match (uu___272_444) with
...
let eff_to_string : FStar_Extraction_ML_Syntax.e_tag  ->  Prims.string = (fun uu___273_510 -> (match (uu___273_510) with
...
let is_type_abstraction : 'a 'b 'c . (('a, 'b) FStar_Util.either * 'c) Prims.list  ->  Prims.bool = (fun uu___274_1243 -> (match (uu___274_1243) with
...
let is_xtuple : (Prims.string Prims.list * Prims.string)  ->  Prims.int FStar_Pervasives_Native.option = (fun uu____1309 -> (match (uu____1309) with
| (ns, n1) -> begin
(

let uu____1331 = (

let uu____1333 = (FStar_Util.concat_l "." (FStar_List.append ns ((n1)::[])))
in (FStar_Parser_Const.is_tuple_datacon_string uu____1333))
...
let uu____1345 = (FStar_Util.char_at n1 (Prims.parse_int "7"))
in (FStar_Util.int_of_char uu____1345))
in FStar_Pervasives_Native.Some (uu____1343))
end
| uu____1349 -> begin
FStar_Pervasives_Native.None
end))
...
let record_field_path : FStar_Ident.lident Prims.list  ->  Prims.string Prims.list = (fun uu___275_1386 -> (match (uu___275_1386) with
| (f)::uu____1393 -> begin
(

let uu____1396 = (FStar_Util.prefix f.FStar_Ident.ns)
...
let is_xtuple_ty : (Prims.string Prims.list * Prims.string)  ->  Prims.int FStar_Pervasives_Native.option = (fun uu____1486 -> (match (uu____1486) with
| (ns, n1) -> begin
(

let uu____1508 = (

let uu____1510 = (FStar_Util.concat_l "." (FStar_List.append ns ((n1)::[])))
in (FStar_Parser_Const.is_tuple_constructor_string uu____1510))
...
let uu____1522 = (FStar_Util.char_at n1 (Prims.parse_int "5"))
in (FStar_Util.int_of_char uu____1522))
in FStar_Pervasives_Native.Some (uu____1520))
end
| uu____1526 -> begin
FStar_Pervasives_Native.None
end))
...
let uu____1541 = (is_xtuple_ty mlp)
...
let flatten_mlpath : (Prims.string Prims.list * Prims.string)  ->  Prims.string = (fun uu____1588 -> (match (uu____1588) with
| (ns, n1) -> begin
(

let uu____1608 = (codegen_fsharp ())
...
let uu____1636 = (FStar_All.pipe_right l.FStar_Ident.ns (FStar_List.map (fun i -> i.FStar_Ident.idText)))
in ((uu____1636), (l.FStar_Ident.ident.FStar_Ident.idText))))


let rec erasableType : unfold_t  ->  FStar_Extraction_ML_Syntax.mlty  ->  Prims.bool = (fun unfold_ty t -> (

let uu____1670 = (FStar_Extraction_ML_UEnv.erasableTypeNoDelta t)
...
let uu____1714 = (eraseTypeDeep unfold_ty tyd)
in (

let uu____1718 = (eraseTypeDeep unfold_ty tycd)
in ((uu____1714), (etag), (uu____1718))))
in FStar_Extraction_ML_Syntax.MLTY_Fun (uu____1707))
end
| uu____1722 -> begin
t
end)
...
let uu____1730 = (erasableType unfold_ty t)
...
in ((uu____1745), (mlp)))
in FStar_Extraction_ML_Syntax.MLTY_Named (uu____1738))
end))
end
| FStar_Extraction_ML_Syntax.MLTY_Tuple (lty) -> begin
(

let uu____1756 = (FStar_List.map (eraseTypeDeep unfold_ty) lty)
in FStar_Extraction_ML_Syntax.MLTY_Tuple (uu____1756))
end
| uu____1762 -> begin
t
end))
...
let uu____1778 = (mk_ty_fun (((("x"), (FStar_Extraction_ML_Syntax.ml_bool_ty)))::((("y"), (FStar_Extraction_ML_Syntax.ml_bool_ty)))::[]) FStar_Extraction_ML_Syntax.ml_bool_ty)
in (FStar_Extraction_ML_Syntax.with_ty uu____1778))
in (FStar_All.pipe_left uu____1773 (FStar_Extraction_ML_Syntax.MLE_Name (((("Prims")::[]), ("op_AmpAmp"))))))
...
let uu____1882 = (FStar_Range.file_of_range r)
in ((line), (uu____1882))))))
...
| FStar_Extraction_ML_Syntax.MLTY_Fun (a, uu____1905, b) -> begin
(

let uu____1907 = (doms_and_cod b)
...
| FStar_Extraction_ML_Syntax.MLTY_Fun (a, uu____1969, b) -> begin
(

let uu____1971 = (uncurry_mlty_fun b)
...
| NoTacticEmbedding (uu____2008) -> begin
true
end
| uu____2011 -> begin
false
...
| NoTacticEmbedding (uu____2022) -> begin
uu____2022
end))
...
let uu____2050 = (FStar_Util.format2 "Plugin %s will not run natively because %s.\n" t msg)
in ((FStar_Errors.Warning_CallNotImplementedAsWarning), (uu____2050)))
in (FStar_Errors.log_issue r uu____2044)))
...
let uu____2168 = (FStar_Extraction_ML_Syntax.mlpath_of_lident l)
in FStar_Extraction_ML_Syntax.MLE_Name (uu____2168))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) uu____2167)))
in (
...
let uu____2176 = (FStar_Extraction_ML_Syntax.mlpath_of_lident l)
in FStar_Extraction_ML_Syntax.MLE_Name (uu____2176))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) uu____2175)))
in (

let str_to_name = (fun s -> (

let uu____2185 = (FStar_Ident.lid_of_str s)
in (lid_to_name uu____2185)))
in (

let str_to_top_name = (fun s -> (

let uu____2194 = (FStar_Ident.lid_of_str s)
in (lid_to_top_name uu____2194)))
in (
...
let uu____2240 = (str_to_name "FStar_Ident.lid_of_str")
in (
...
in FStar_Extraction_ML_Syntax.MLC_String (uu____2248))
in FStar_Extraction_ML_Syntax.MLE_Const (uu____2247))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) uu____2246))
in (uu____2245)::[])
in ((uu____2240), (uu____2242))))
in FStar_Extraction_ML_Syntax.MLE_App (uu____2233))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) uu____2232))
in (

let emb_prefix = (fun uu___276_2263 -> (match (uu___276_2263) with
| Syntax_term -> begin
...
let uu____2319 = (FStar_Util.string_of_int arity)
in (Prims.strcat "arrow_as_prim_step_" uu____2319))
in (emb_prefix l uu____2317)))
in (
...
let uu____2343 = (emb_prefix l "mk_any_emb")
in (

let uu____2345 = (

let uu____2348 = (str_to_name s)
in (uu____2348)::[])
in ((uu____2343), (uu____2345))))
in FStar_Extraction_ML_Syntax.MLE_App (uu____2336))
in (FStar_All.pipe_left w uu____2335)))
in (
...
let uu____2406 = (emb_prefix l "e_arrow")
in ((uu____2406), ((e1)::(e2)::[])))
in FStar_Extraction_ML_Syntax.MLE_App (uu____2399))
in (FStar_All.pipe_left w uu____2398)))
in (
...
let uu____2571 = (FStar_Parser_Const.mk_tuple_lid (Prims.parse_int "2") FStar_Range.dummyRange)
in ((uu____2571), ((Prims.parse_int "2")), ("tuple2")))
in ((uu____2562), (Syntax_term)))
in (
...
let uu____2622 = (FStar_Reflection_Data.fstar_refl_types_lid "term")
in ((uu____2622), ((Prims.parse_int "0")), ("term")))
in ((uu____2613), (Refl_emb)))
in (
...
let uu____2673 = (FStar_Reflection_Data.fstar_refl_types_lid "fv")
in ((uu____2673), ((Prims.parse_int "0")), ("fv")))
in ((uu____2664), (Refl_emb)))
in (
...
let uu____2724 = (FStar_Reflection_Data.fstar_refl_types_lid "binder")
in ((uu____2724), ((Prims.parse_int "0")), ("binder")))
in ((uu____2715), (Refl_emb)))
in (
...
let uu____2775 = (FStar_Reflection_Data.fstar_refl_syntax_lid "binders")
in ((uu____2775), ((Prims.parse_int "0")), ("binders")))
in ((uu____2766), (Refl_emb)))
in (
...
let uu____2826 = (FStar_Reflection_Data.fstar_refl_data_lid "exp")
in ((uu____2826), ((Prims.parse_int "0")), ("exp")))
in ((uu____2817), (Refl_emb)))
in (uu____2804)::[])
in (uu____2753)::uu____2789))
in (uu____2702)::uu____2738))
in (uu____2651)::uu____2687))
in (uu____2600)::uu____2636))
in (uu____2549)::uu____2585))
in (((((FStar_Parser_Const.option_lid), ((Prims.parse_int "1")), ("option"))), (Syntax_term)))::uu____2534)
in (((((FStar_Parser_Const.list_lid), ((Prims.parse_int "1")), ("list"))), (Syntax_term)))::uu____2519)
in (((((FStar_Parser_Const.norm_step_lid), ((Prims.parse_int "0")), ("norm_step"))), (Syntax_term)))::uu____2504)
in (((((FStar_Parser_Const.string_lid), ((Prims.parse_int "0")), ("string"))), (Syntax_term)))::uu____2489)
in (((((FStar_Parser_Const.unit_lid), ((Prims.parse_int "0")), ("unit"))), (Syntax_term)))::uu____2474)
in (((((FStar_Parser_Const.bool_lid), ((Prims.parse_int "0")), ("bool"))), (Syntax_term)))::uu____2459)
in (((((FStar_Parser_Const.int_lid), ((Prims.parse_int "0")), ("int"))), (Syntax_term)))::uu____2444)
in (

let nbe_cs = (FStar_List.map (fun uu___277_3133 -> (match (uu___277_3133) with
| (x, Syntax_term) -> begin
((x), (NBE_t))
end
| (x, Refl_emb) -> begin
...
in (fun uu___278_3234 -> (match (uu___278_3234) with
| Syntax_term -> begin
term_cs
end
| Refl_emb -> begin
...
let is_known_type_constructor = (fun l fv1 n1 -> (FStar_Util.for_some (fun uu____3286 -> (match (uu____3286) with
| ((x, args, uu____3302), uu____3303) -> begin
((FStar_Syntax_Syntax.fv_eq_lid fv1 x) && (Prims.op_Equality n1 args))
end)) (known_type_constructors l)))
in (

let find_env_entry = (fun bv uu____3333 -> (match (uu____3333) with
| (bv', uu____3341) -> begin
...
let uu____3376 = (FStar_Syntax_Subst.compress t3)
in uu____3376.FStar_Syntax_Syntax.n)
...
let uu____3393 = (FStar_Util.find_opt (find_env_entry bv) env)
in (FStar_Util.must uu____3393))
in (FStar_Pervasives_Native.snd uu____3387))
in (FStar_All.pipe_left (mk_any_embedding l) uu____3385))
end
| FStar_Syntax_Syntax.Tm_refine (x, uu____3414) -> begin
(mk_embedding l env x.FStar_Syntax_Syntax.sort)
end
| FStar_Syntax_Syntax.Tm_ascribed (t4, uu____3420, uu____3421) -> begin
(mk_embedding l env t4)
end
| FStar_Syntax_Syntax.Tm_arrow ((b)::[], c) when (FStar_Syntax_Util.is_pure_comp c) -> begin
(

let uu____3494 = (FStar_Syntax_Subst.open_comp ((b)::[]) c)
...
let uu____3517 = (FStar_List.hd bs)
in (FStar_Pervasives_Native.fst uu____3517))
in uu____3516.FStar_Syntax_Syntax.sort)
in (

let t11 = (FStar_Syntax_Util.comp_result c1)
in (

let uu____3535 = (mk_embedding l env t0)
in (

let uu____3536 = (mk_embedding l env t11)
in (emb_arrow l uu____3535 uu____3536)))))
end))
end
...
in (((b)::[]), (uu____3630)))
in FStar_Syntax_Syntax.Tm_arrow (uu____3615))
in (FStar_Syntax_Syntax.mk uu____3614))
in (uu____3607 FStar_Pervasives_Native.None t3.FStar_Syntax_Syntax.pos))
in (mk_embedding l env t4)))
end
| FStar_Syntax_Syntax.Tm_fvar (uu____3658) -> begin
(

let uu____3659 = (FStar_Syntax_Util.head_and_args t3)
...
let arg_embeddings = (FStar_All.pipe_right args (FStar_List.map (fun uu____3742 -> (match (uu____3742) with
| (t4, uu____3750) -> begin
(mk_embedding l env t4)
end))))
in (
...
let uu____3770 = (FStar_Util.find_opt (fun uu____3802 -> (match (uu____3802) with
| ((x, uu____3817, uu____3818), uu____3819) -> begin
(FStar_Syntax_Syntax.fv_eq_lid fv1 x)
end)) (known_type_constructors l))
in (FStar_All.pipe_right uu____3770 FStar_Util.must))
in (match (uu____3757) with
| ((uu____3870, t_arity, _trepr_head), loc_embedding) -> begin
(
...
let uu____3895 = (FStar_Syntax_Print.term_to_string t3)
in (Prims.strcat "Embedding not defined for type " uu____3895))
in NoTacticEmbedding (uu____3893))
in (FStar_Exn.raise uu____3892))
end)))
end))
end
| FStar_Syntax_Syntax.Tm_uinst (uu____3898) -> begin
(

let uu____3905 = (FStar_Syntax_Util.head_and_args t3)
...
let arg_embeddings = (FStar_All.pipe_right args (FStar_List.map (fun uu____3988 -> (match (uu____3988) with
| (t4, uu____3996) -> begin
(mk_embedding l env t4)
end))))
in (
...
let uu____4016 = (FStar_Util.find_opt (fun uu____4048 -> (match (uu____4048) with
| ((x, uu____4063, uu____4064), uu____4065) -> begin
(FStar_Syntax_Syntax.fv_eq_lid fv1 x)
end)) (known_type_constructors l))
in (FStar_All.pipe_right uu____4016 FStar_Util.must))
in (match (uu____4003) with
| ((uu____4116, t_arity, _trepr_head), loc_embedding) -> begin
(
...
let uu____4141 = (FStar_Syntax_Print.term_to_string t3)
in (Prims.strcat "Embedding not defined for type " uu____4141))
in NoTacticEmbedding (uu____4139))
in (FStar_Exn.raise uu____4138))
end)))
end))
end
| FStar_Syntax_Syntax.Tm_app (uu____4144) -> begin
(

let uu____4161 = (FStar_Syntax_Util.head_and_args t3)
...
in uu____4214.FStar_Syntax_Syntax.n)
in (match (uu____4213) with
| FStar_Syntax_Syntax.Tm_fvar (fv1) when (is_known_type_constructor l fv1 n_args) -> begin
(

let arg_embeddings = (FStar_All.pipe_right args (FStar_List.map (fun uu____4244 -> (match (uu____4244) with
| (t4, uu____4252) -> begin
(mk_embedding l env t4)
end))))
in (
...
let uu____4272 = (FStar_Util.find_opt (fun uu____4304 -> (match (uu____4304) with
| ((x, uu____4319, uu____4320), uu____4321) -> begin
(FStar_Syntax_Syntax.fv_eq_lid fv1 x)
end)) (known_type_constructors l))
in (FStar_All.pipe_right uu____4272 FStar_Util.must))
in (match (uu____4259) with
| ((uu____4372, t_arity, _trepr_head), loc_embedding) -> begin
(
...
let uu____4397 = (FStar_Syntax_Print.term_to_string t3)
in (Prims.strcat "Embedding not defined for type " uu____4397))
in NoTacticEmbedding (uu____4395))
in (FStar_Exn.raise uu____4394))
end)))
end))
end
| uu____4400 -> begin
...
in (Prims.strcat "Embedding not defined for type " uu____4404))
in NoTacticEmbedding (uu____4402))
in (FStar_Exn.raise uu____4401))
end))))
in (
...
let uu____4434 = (str_to_name "FStar_Syntax_Embeddings.debug_wrap")
in (
...
let uu____4442 = (FStar_Ident.string_of_lid fv_lid1)
in FStar_Extraction_ML_Syntax.MLC_String (uu____4442))
in FStar_Extraction_ML_Syntax.MLE_Const (uu____4441))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) uu____4440))
in (
...
let uu____4460 = (str_to_name "args")
in (uu____4460)::[])
in ((body), (uu____4457)))
in FStar_Extraction_ML_Syntax.MLE_App (uu____4450))
in (FStar_All.pipe_left w uu____4449))
in (mk_lam "_" uu____4448))
in (uu____4447)::[])
in (uu____4439)::uu____4444))
in ((uu____4434), (uu____4436))))
in FStar_Extraction_ML_Syntax.MLE_App (uu____4427))
in (FStar_All.pipe_left w uu____4426))
...
let uu____4529 = (str_to_name "args")
in (uu____4529)::[])
in ((body), (uu____4526)))
in FStar_Extraction_ML_Syntax.MLE_App (uu____4519))
in (FStar_All.pipe_left w uu____4518))
in ((pattern), (FStar_Pervasives_Native.None), (uu____4517)))
in (
...
let uu____4553 = (str_to_name "failwith")
in (
...
let uu____4559 = (mlexpr_of_const FStar_Range.dummyRange (FStar_Const.Const_string ((("arity mismatch"), (FStar_Range.dummyRange)))))
in (FStar_All.pipe_left w uu____4559))
in (uu____4558)::[])
in ((uu____4553), (uu____4555))))
in FStar_Extraction_ML_Syntax.MLE_App (uu____4546))
in (FStar_All.pipe_left w uu____4545))
in ((FStar_Extraction_ML_Syntax.MLP_Wild), (FStar_Pervasives_Native.None), (uu____4544)))
in (
...
let uu____4583 = (str_to_name "args")
in ((uu____4583), ((branch1)::(default_branch)::[])))
in FStar_Extraction_ML_Syntax.MLE_Match (uu____4568))
in (FStar_All.pipe_left w uu____4567))
in (
...
let uu____4628 = (str_to_name "FStar_Syntax_Embeddings.debug_wrap")
in (
...
let uu____4636 = (FStar_Ident.string_of_lid fv_lid1)
in FStar_Extraction_ML_Syntax.MLC_String (uu____4636))
in FStar_Extraction_ML_Syntax.MLE_Const (uu____4635))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) uu____4634))
in (

let uu____4638 = (

let uu____4641 = (mk_lam "_" body1)
in (uu____4641)::[])
in (uu____4633)::uu____4638))
in ((uu____4628), (uu____4630))))
in FStar_Extraction_ML_Syntax.MLE_App (uu____4621))
in (FStar_All.pipe_left w uu____4620))
in (mk_lam "args" body2)))))))))
end))
in (

let uu____4646 = (FStar_Syntax_Util.arrow_formals_comp t1)
in (match (uu____4646) with
| (bs, c) -> begin
(

let uu____4679 = (match (arity_opt) with
| FStar_Pervasives_Native.None -> begin
((bs), (c))
end
| FStar_Pervasives_Native.Some (n1) -> begin
...
let uu____4780 = (FStar_Util.first_N n1 bs)
...
let uu____4858 = (FStar_Syntax_Util.arrow rest c)
in (FStar_All.pipe_left FStar_Syntax_Syntax.mk_Total uu____4858))
in ((bs1), (c1)))
end))
end
| uu____4871 -> begin
...
let uu____4875 = (FStar_Ident.string_of_lid fv_lid1)
in (

let uu____4877 = (FStar_Util.string_of_int n1)
in (

let uu____4879 = (FStar_Util.string_of_int n_bs)
in (FStar_Util.format3 "Embedding not defined for %s; expected arity at least %s; got %s" uu____4875 uu____4877 uu____4879))))
...
let uu____4957 = (FStar_Util.prefix_until (fun uu____4999 -> (match (uu____4999) with
| (b, uu____5008) -> begin
(

let uu____5013 = (

let uu____5014 = (FStar_Syntax_Subst.compress b.FStar_Syntax_Syntax.sort)
in uu____5014.FStar_Syntax_Syntax.n)
in (match (uu____5013) with
| FStar_Syntax_Syntax.Tm_type (uu____5018) -> begin
false
end
| uu____5020 -> begin
true
...
let uu____5262 = (FStar_Util.string_of_int i)
in (Prims.strcat "tv_" uu____5262))) type_vars)
in (
...
let uu____5362 = (FStar_Syntax_Util.is_pure_comp c1)
...
let uu____5389 = (lid_to_top_name fv_lid2)
in (
...
let uu____5408 = (FStar_Util.string_of_int tvar_arity)
in ((uu____5408), (FStar_Pervasives_Native.None)))
in FStar_Extraction_ML_Syntax.MLC_Int (uu____5396))
in FStar_Extraction_ML_Syntax.MLE_Const (uu____5395))
in (FStar_All.pipe_left (FStar_Extraction_ML_Syntax.with_ty FStar_Extraction_ML_Syntax.MLTY_Top) uu____5394))
in (uu____5393)::(fv_lid_embedded)::(cb)::[])
in (uu____5389)::uu____5390))
in (res_embedding)::uu____5386)
in (FStar_List.append arg_unembeddings uu____5383))
in (
...
let uu____5433 = (match ((Prims.op_Equality loc NBE_t)) with
| true -> begin
cb_tabs
end
| uu____5435 -> begin
...
let uu____5449 = (FStar_TypeChecker_Env.norm_eff_name tcenv (FStar_Syntax_Util.comp_effect_name c1))
in (FStar_Ident.lid_equals uu____5449 FStar_Parser_Const.effect_TAC_lid))
...
let uu____5462 = (FStar_Util.string_of_int non_tvar_arity)
in (Prims.strcat (mk_tactic_interpretation loc) uu____5462))
in (str_to_top_name uu____5460))
in (
...
let uu____5482 = (FStar_Util.string_of_int non_tvar_arity)
in (Prims.strcat (mk_from_tactic loc) uu____5482))
in (str_to_top_name uu____5480))
in (

let uu____5490 = (

let uu____5493 = (lid_to_top_name fv_lid2)
in (uu____5493)::[])
in ((uu____5479), (uu____5490))))
in FStar_Extraction_ML_Syntax.MLE_App (uu____5472))
in (FStar_All.pipe_left w uu____5471))
in (
...
let uu____5507 = (FStar_All.pipe_left w (FStar_Extraction_ML_Syntax.MLE_App (((h), ((FStar_List.append args ((all_args)::[])))))))
in (mk_lam "args" uu____5507))
end
| uu____5511 -> begin
(

let uu____5515 = (FStar_All.pipe_left w (FStar_Extraction_ML_Syntax.MLE_App (((h), (args)))))
in (abstract_tvars tvar_names uu____5515))
end)
in (

let uu____5518 = (

let uu____5519 = (mk_lam "ncb" tabs)
in (mk_lam "psc" uu____5519))
in ((uu____5518), ((arity + (Prims.parse_int "1"))), (false))))))))))
end
| uu____5532 -> begin
(
...
let uu____5537 = (FStar_Syntax_Print.term_to_string t1)
in (Prims.strcat "Plugins not defined for type " uu____5537))
in NoTacticEmbedding (uu____5535))
in (FStar_Exn.raise uu____5534))
end))
end)))))
end
| ((b, uu____5549))::bs4 -> begin
(

let uu____5569 = (

let uu____5572 = (mk_embedding loc tvar_context b.FStar_Syntax_Syntax.sort)
in (uu____5572)::accum_embeddings)
in (aux loc uu____5569 bs4))
end))
in (FStar_All.try_with (fun uu___282_5594 -> (match (()) with
| () -> begin
(
...
| (w', uu____5657, uu____5658) -> begin
FStar_Pervasives_Native.Some (((w1), (w'), (a), (b)))
end))
end))
end)) (fun uu___281_5678 -> (match (uu___281_5678) with
| NoTacticEmbedding (msg) -> begin
((

let uu____5694 = (FStar_Syntax_Print.fv_to_string fv)
in (not_implemented_warning t1.FStar_Syntax_Syntax.pos uu____5694 msg));
FStar_Pervasives_Native.None;
)
end))))))))
end))))
